...
Item {
  ...
  function recomputeFiltered() {
    ...
    var out = []
    for (var i = 0; i < options.length; i++) {
      var labelText = optionLabel(options[i]).toLowerCase()
      var descriptionText = optionDescription(options[i]).toLowerCase()
      if (labelText.indexOf(q) !== -1 || descriptionText.indexOf(q) !== -1) out.push(options[i])
    }
    filtered = out
  }
  ...
  Column {
    ...
    BorderSurface {
      ...
      QQC.Popup {
        ...
        property real belowY: 0
        property real aboveBottomY: 0
        property real availableBelow: 0
        property real availableAbove: 0
        readonly property real idealHeight: Math.max(root.popupMinHeight,
          Math.min(resultList.contentHeight + searchHeader.height + Style.spacing.xxs + 1,
            root.popupRowHeight * 6 + 5 * Style.spacing.labelGap + searchHeader.height + Style.spacing.xxs + 1))
        readonly property bool openAbove: root.preferUp || (availableBelow < Math.min(idealHeight, root.popupMinHeight) && availableAbove > availableBelow)
        readonly property real fittedSpace: openAbove ? availableAbove : availableBelow
        ...
        function reposition() {
          ...
          anchorX = top.x
          belowY = below.y
          aboveBottomY = top.y - Style.spacing.xxs
          availableBelow = Math.max(0, parent.height - belowY - Style.space(12))
          availableAbove = Math.max(0, aboveBottomY - Style.space(12))
        }

        x: anchorX
        y: openAbove ? Math.max(Style.space(6), aboveBottomY - height) : belowY
        z: 10000
        width: trigger.width
        implicitHeight: Math.max(Math.min(Style.space(80), idealHeight), Math.min(fittedSpace, idealHeight))
